...
export function WorshipPage() {
  ...
  const WORSHIP_STORAGE_KEY = "ramadan_loop_worship_tasks";

  const [tasks, setTasks] = useState(() => {
    const saved = localStorage.getItem(WORSHIP_STORAGE_KEY);
    if (saved) {
      try {
        const parsed = JSON.parse(saved);
        // Map back the icons since they can't be stringified
        ...
        console.error("Failed to parse saved worship tasks", e);
      }
    }
    return [
      {
  ...
  const completedCount = tasks.filter((t: any) => t.checked).length;
  const progress = (completedCount / tasks.length) * 100;

  useEffect(() => {
    // Save to localStorage whenever tasks change
    const toSave = tasks.map((t: any) => ({
      id: t.id,
      label: t.label,
      checked: t.checked,
    }));
    localStorage.setItem(WORSHIP_STORAGE_KEY, JSON.stringify(toSave));
  ...
  }, [completedCount, completeStep, navigate, tasks]);
